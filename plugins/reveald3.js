 ...
 * note: I have added an export at the bottom of this file to make it importable
 */

const Reveald3 = window.Reveald3 || {
  id: 'Reveald3',
  init (deck) {
    initReveald3(deck)
  }
...
const initReveald3 = function (Reveal) {
  ...
  const getIframeStyle = (viz) => {
    ...
    while ((matchStyleArray = regexStyle.exec(dataStyleString)) !== null) {
  ...
  async function initialize (vizObject) {
    const {
      isBackground, onCurrentSlide, slide, container, file, fragmentsInSlide, preload,
      iframeStyle, iframeExtra
    } = vizObject
    ...
    iframe.addEventListener('load', function () {
      ...
      const nodeList = getAllIframes(slide)
      const allVisualizationSteps = []
  ...
  function generateVisualizationStepsIndices (allVisualizationSteps, slideFragmentSteps) {
    ...
    const uniqueAllVisualizationIndices = [...new Set([].concat(...allVisualizationIndices))]
    ...
    const allVisualizationStepsIndices = []
    for (let i = 0; i < allVisualizationSteps.length; i++) {
      ...
      if ((visualizationSteps) && (visualizationIndices)) {
        ...
        const visualizationStepsIndices = {}
  ...
  function triggerTransition (iframe, currentStep, direction) {
    ...
    } else if ((iframe.transitionSteps) && (iframe.transitionSteps[currentStep])) {
  ...
  function handleFragments (event, direction) {
    ...
    const currentStep = parseInt(event.fragments[0].getAttribute('data-fragment-index'))
    // forward transition
    const slide = event.fragment.closest('section')
    // get all iframe embedding visualisations
    const allIframes = getAllIframes(slide)
